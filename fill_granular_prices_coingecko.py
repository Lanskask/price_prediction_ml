"""Fill Bitcoin price columns with granular prices using CoinGecko API."""
...
def fetch_coingecko_hourly_prices(start_date: datetime, end_date: datetime) -> pd.DataFrame:
    """
    Fetch hourly Bitcoin prices from CoinGecko API.

    CoinGecko provides hourly data for up to 90 days per request.
    ...
    """
    print(f"\nFetching hourly Bitcoin price data from CoinGecko API...")
    print(f"Date range: {start_date} to {end_date}")
    print(f"Time span: {(end_date - start_date).days} days")

    all_data = []
    current_start = start_date
    chunk_days = 89  # CoinGecko allows 90 days of hourly data per request
    chunk_num = 1
    ...
    while current_start < end_date:
        ...
        try:
            ...
            if 'prices' in data and data['prices']:
                ...
                # CoinGecko doesn't provide OHLC in the free API, so we'll use price as all values
                # For better accuracy, we'd need the Pro API, but this works for our purposes
                ...
                if 'total_volumes' in data and data['total_volumes']:
                    ...
                    df = df.rename(columns={'volume': 'btc_volume'})
                else:
            ...
            time.sleep(7)  # ~8 requests per minute to be safe
    ...
    print(f"\n✓ Total price points fetched: {len(combined_df):,}")
...
def add_ml_targets(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Calculate volatility - since CoinGecko gives us same values, calculate based on recent prices
...
def fill_granular_prices(input_file: str, output_file: str = None):
    """
    Fill Bitcoin price columns with granular prices using CoinGecko.
    ...
    """
    print("="*80)
    print("FILLING GRANULAR BITCOIN PRICE DATA (CoinGecko API)")
    print("="*80)
    ...
    num_chunks = total_days // 89 + 1
    estimated_minutes = num_chunks * 0.12  # ~7 seconds per chunk
    print(f"\nEstimated fetch time: ~{estimated_minutes:.1f} minutes ({num_chunks} API requests)")
    print("Note: CoinGecko free tier has rate limits. Please be patient...")

    # Fetch Bitcoin prices from CoinGecko
    price_df = fetch_coingecko_hourly_prices(min_date, max_date)
    ...
    # Drop price_time_diff_minutes if user doesn't need it (optional)
    # merged_df = merged_df.drop('price_time_diff_minutes', axis=1, errors='ignore')
    ...
    # Show sample
    print("\nSample data (first 5 rows with timestamps):")
    ...
    print("\n" + "="*80)
    print("✓ DATASET NOW HAS GRANULAR BITCOIN PRICES!")
    print("="*80)
    print("\nNote: Prices are matched to the nearest timestamp of article publication.")
    print("The 'price_time_diff_minutes' column shows the time difference in minutes.")
...
if __name__ == "__main__":
    ...
    if success:
        print("\n✓ Success! Dataset now has prices matched to article timestamps.")
        print("\nNext steps:")
        print("1. The dataset now has granular prices from CoinGecko API")
        print("2. Start building your ML model with this time-accurate price data!")
    else:
